...
class NeRF(nn.Module):
    ...
    def forward(self, x, viewdirs=None):
        ...
        for i, l in enumerate(self.pts_linears):
            ...
            h = self.relu_layer(h) # HINT: use relu
            if i in self.skips:
                h = torch.cat((input_pts,h),axis=-1) # implement skip with torch.cat

        if self.d_viewdirs is not None:
            alpha = self.alpha_linear(h) # HINT: feed h to alpha linear
            feature = self.feature_linear(h) # HINT: feed h to feature linear
            h = torch.cat((feature,input_views),axis=-1) # HINT: concat feature and input_views to create the input for the views_linreas
            ...
            outputs = torch.cat((rgb,alpha),axis=-1) # HINT: concat rgb and alpha
        else:
...
class Embedder(nn.Module):
    ...
    def __init__(self, d_input, n_freqs, log_space=False):
        ...
        for freq in freq_bands:
            self.embed_fns.append(lambda x, freq=freq: torch.sin(x * freq))# HINT: use torch.sin
            self.embed_fns.append(lambda x, freq=freq: torch.cos(x * freq)) # HINT: use torch.cos

    def forward(self, x):
        """
        Apply positional encoding to input.
        """
        return torch.cat([fn(x) for fn in self.embed_fns], dim=-1) #hardcoded to cuda for now...
