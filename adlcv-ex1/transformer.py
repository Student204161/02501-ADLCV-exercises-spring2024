...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        keys = keys.view(batch_size, seq_length, self.num_heads, self.head_dim) # using .view() for reshaping without copying data
        # Secondly merge the batch_size with the num_heads
        keys = rearrange(keys, 'b s h d -> (b h) s d') # using rearrange() to merge the batch_size with the num_heads
        ...
        attention_logits = torch.matmul(queries,keys.transpose(1,2)) # multiply queries and keys
